...
# --- Employer RRSP match CRUD ---

class EmployerRRSPCreate(BaseModel):
    income_stream_id: int
    rrsp_account_id: int
    employee_rate: float        # % of gross pay employee contributes
    employer_match_rate: float  # % of gross pay employer adds
...
def delete_employer_rrsp(
    ...
    if not obj:
        raise HTTPException(status_code=404, detail="Employer RRSP match not found")
    db.delete(obj)
    db.commit()
