...
def update_recurring(rule_id: int, data: RecurringUpdate, db: Session = Depends(get_db)):
...
def skip_next(rule_id: int, db: Session = Depends(get_db)):
...
@router.post("/execute")
def execute_due(db: Session = Depends(get_db)):
